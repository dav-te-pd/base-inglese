...
| ⚠️ *(aggiornamento a `resolveEpisodeOrder`, 2026-09-26)* | dal passo della lista degli episodi **toglie anche gli SPENTI** (`CONFIG.episodiSpenti`) | — | ⚠️ **Che «spento» sia una DICHIARAZIONE e non l'assenza dalla lista:** un episodio che la sequenza non nomina resta raggiungibile **in coda**, quindi se l'assenza valesse come spegnimento un episodio nuovo **nascerebbe spento**. ⚠️ **E che spegnerli TUTTI non svuoti l'app in silenzio:** in quel caso torna l'ordine **intero** con un `errore` che li nomina — *una lista vuota è la schermata che sembra rotta senza dire perché, e la ragione sarebbe una manopola.* **Un id spento che non esiste NON è un errore**: è il caso normale di chi spegne un episodio e poi lo rinomina. |
...
| `FORME` | I cinque comportamenti del riconoscimento, per nome. | → `['auto', 'suStop', 'manuale', 'muto', 'continuo']` | ⚠️ **Che la differenza fra le quattro sia QUANDO arriva `onend`**, non un dettaglio di forma: `auto` lo manda insieme al risultato, `suStop` solo dopo `stop()`, `manuale` manda `onstart` e non produce mai risultati, `muto` non fa niente su `start()` (premi e non parli). *È esattamente l'ordine degli eventi asincroni che la regola 19 dice di non semplificare — un nome sbagliato qui non dà un errore, dà un verde.* Un nome fuori elenco **alza**, non ripiega. |
| `spiaToni` | Registra ogni tono di Web Audio in `window.__playedTones` — i suoni dell'app (Traguardo, Uscita, countdown del Dialogo, mini-ascolto). | *niente* — è `{ content }`, si passa ad `addInitScript` o si installa dopo con `addScriptTag({ content: spiaToni.content })` | ⚠️ **Che NON sia un'opzione di `mockBrowser`, ed è voluto:** sei file la usano **senza** il finto del sintetizzatore. ⚠️ **Che `t: performance.now()` ci sia sempre** (viene da `dialogo_extra`) e che l'unico gancio sia `window.__annotaTono`, con **un solo utente**: `batch15`, che deve sapere se la schermata finale era già visibile **nell'istante del tono**. Chiude **tredici** copie. |
| `catturaAvvisi` | Raccoglie i `console.warn` in `window.__consoleWarnings`. | *niente* — è `{ content }` | Che l'originale venga chiamato lo stesso: **non zittisce**, così un avviso resta nel log della corsa oltre che leggibile. Chiude **tre** copie identiche. |
| `componi(...pezzi)` | Mette più pezzi in un solo `addInitScript`. | `({content}, {content}, …)` → `{ content }` | Che serva dove il finto e la spia erano **un** blocco: i **sessanta** punti di chiamata non si toccano, e i pezzi restano installati nello stesso istante di prima. |

## `tests/boot.js` — entrare nell'app come utente, dal lato dei test

*Nasce il 2026-09-25 (passo F.4 ②). **Trentadue file si scrivevano il proprio
avvio, e venticinque avevano lo SCHELETRO IDENTICO**: cambiava solo cosa
scrivevano nel `localStorage` prima di entrare.*

| Pezzo | Cosa fa | Cosa gli passi → cosa torna | Cosa dà per scontato |
|---|---|---|---|
| `bootUtente(page, opzioni)` | Entra nell'app col nome dato e apre la mappa dell'episodio, avendo prima seminato progressi e introduzioni. | `{ utente, completati, introChiuse, storage, episodio, personalizzaVista, base }` → *niente* | ⚠️ **Che lo scheletro sia quello dei venticinque, ATTESE COMPRESE** — `waitForTimeout` e tutto. *Mescolare «unifico» e «rendo robuste le attese» renderebbe illeggibile il rosso: quelle attese a tempo restano un debito **dichiarato**, non un debito nuovo.* ⚠️ Che `attendiPrimaSchermata` venga **prima** di guardare se c'è il campo del nome: senza, «non c'è il campo» e «non c'è ancora niente» si leggono uguali (passo 1.11b). ⚠️ Che al secondo giro il nome sia già salvato e la schermata di benvenuto **non compaia**: per questo guarda cosa c'è a schermo invece di darlo per scontato. |
| `INTRO_DI_TUTTI` | I quindici `kind` che hanno una schermata di introduzione, più `mappaEpisodio` e `personalizzazione`. | → lista di 15 stringhe | ⚠️ **CHE NON SIA `stepIds()`, E NON È UNA SVISTA.** `stepIds()` dà i **22 PASSI** di una sequenza coi loro suffissi (`matchEngIta-2`, `flashcardAEngIta`); la chiave dell'intro è per **KIND**. *Misurato: nella lista e non in `stepIds()` ci sono `mappaEpisodio` e `flashcard`; in `stepIds()` e non nella lista ce ne sono nove.* Sono due elenchi di cose diverse, e farne uno solo sarebbe un errore travestito da deduplicazione. Era ricopiata in **ventotto** file. |
| `EPISODIO_PREDEFINITO` | L'episodio su cui i test scrivono i progressi. | → `'gate'` | ⚠️ **Che prima fosse la stringa `'gate'` INCISA in ventisette file.** *È la famiglia del passo 1.19 — «la suite smette di dare per scontato che gate sia il primo» — vista nei test invece che nell'app.* Un file che ne vuole un altro passa `episodio`. |
