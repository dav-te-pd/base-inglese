...
*I nomi e le loro ragioni stanno in `docs/inglese/it/inglese-it-struttura-corso.md`, sezione «I nomi in
...
| | Passo | Stato | Fermata sicura dopo? |
|---|---|---|---|
| **1** | `srShuffle → shuffle` — 11 occorrenze, nessuno stato salvato, nessun DOM, nessun file dati. È il giro di taratura del metodo: se la suite va rossa qui, il problema è il metodo, non la rinomina.<br><br>**FATTO il 2026-09-08.** Undici in `index.html` (la definizione più dieci chiamate) e una in `tests/test_scala_colori.js`. Verificato prima che `shuffle` non collidesse con niente: nel codice non esisteva, nemmeno come parola. Allineate anche le **nove** occorrenze in `docs/validazione.md`, che descrive il codice e avrebbe continuato a nominare una funzione inesistente. **`docs/inglese/it/inglese-it-struttura-corso.md` NON è stato toccato** (regola 33): la sua tabella dichiara la rinomina da fare, ed è la fonte, non un registro di stato. | ☑ | **sì** |
...
⚠️ **Oggi `data/inglese/it/inglese-it-istruzioni-moduli.json` — gruppo C — sta DENTRO
...
## ⚠️ LE SCHEDE CHIUSE DI «COSA C'È DA FARE», SPOSTATE QUI IL 2026-09-26

*Chiesto da chi guida il progetto: **«cerca sempre di sintetizzare e fare in
maniera tabellare. quelle cose vecchie pulisci»**. `decisioni-stato.md` **si
svuota**, questo file **si accumula**: la sezione «cosa c'è da fare» aveva
raccolto novantotto righe in cui le schede CHIUSE — con tutto il loro
ragionamento — stavano in mezzo a quelle aperte, e quattro di loro portavano
numeri che non erano più veri.*

⚠️ **NIENTE È STATO RIASSUNTO NELLO SPOSTAMENTO: qui sotto c'è il blocco
INTERO come stava**, comprese le righe barrate e i numeri sbagliati, perché
*una fotografia corretta non è più una fotografia* — è la stessa ragione per
cui `docs/archivio/` non si aggiorna. **Cosa era sbagliato lo dice la tabella
del double-check in `decisioni-stato.md`**, non una modifica fatta qui.

### Il blocco «DOVE SIAMO — 2026-09-24», come stava
...
### Il blocco «COSA C'È DA FARE — 2026-09-24», come stava
...
| | Cosa |
|---|---|
| **1.18** | ✅ **QUATTRO ASSERZIONI CHIUSE IL 2026-09-25, E LA QUARTA CAMBIA LA DIAGNOSI DELLA SCHEDA.** `[SR Task1]` — la più vecchia della famiglia, nominata il 2026-09-22 — **non cadeva per lentezza**: cadeva davanti al **popup dei tentativi**, uno stato dell'app che l'attesa non conosceva. *Il tetto non c'entrava per un ordine di grandezza: ~610 ms misurati contro 15 s.* ⚠️ **QUINDI «la famiglia di asserzioni che corre contro timer corti» ERA UN NOME SBAGLIATO PER ALMENO UNA DI LORO.** Delle quattro chiuse: una leggeva **prima che i testi arrivassero**, una **misurava sé stessa**, una **correva contro una finestra di 700 ms**, una **aspettava davanti a un modale**. *Quattro cause diverse sotto un'etichetta sola — ed è per questo che «alzare i tetti» non avrebbe chiuso niente.* ✅ **TRE ASSERZIONI CHIUSE IL 2026-09-25, E SONO TRE SPECIE DIVERSE.** ① `test_interruttore_episodio [B]` — leggeva il pulsante di casa **mentre i testi non erano ancora arrivati**; l'app aveva ragione lei. ② `test_attese_condivise [C]` — **misurava sé stessa** (`< 250 ms`): adesso ha un **riferimento** invece di un numero, e la soglia **cresce col carico** (misurato: riferimento 1015 ms, immediati 4 · 33 · 4). ③ `test_batch16 [Job2]` — **correva contro la finestra di 700 ms** dell'audio: adesso attesa, lettura e click stanno **dentro la pagina**, e fra loro non può cadere nessun viaggio. ⚠️ **Il censimento a 20 in parallelo dà ZERO e non vuol dire niente** (1 su 5 non si distingue da 0 su 5); **è a 40 che il difetto si compra**, e lì cadevano queste due. ✅ **PRIMA ASSERZIONE CHIUSA IL 2026-09-25, E IL CENSORE È STATO RIPARATO PRIMA DI LEI.** ⚠️ **Lo strumento non vedeva quattro file su 78** (riga rossa in forma `  FAIL  `, senza trattino): finivano nel secchio `MORTO O ROSSO`, che dice il file e non la riga — *e il primo caso caduto era PROPRIO uno dei quattro.* Corretto riusando la regex che `conta-asserzioni.js` aveva già, e tolto il doppio conteggio che ne è seguito. **Censimento a strumento riparato, 5 giri a 20 in parallelo: UNA sola caduta, nominata** — `test_interruttore_episodio [B] «Il pulsante di casa e il badge in mappa nominano lo stesso episodio»`, 1 su 5. ⚠️ **E l'app aveva ragione lei:** `scriviTestiHome` lascia apposta la parola del markup finché i testi non arrivano, poi riscrive; l'asserzione leggeva nel mezzo. Corretta con l'approdo sul **saluto** (regola 44: nessuna delle due asserzioni lo legge), e **falsificata col guasto realistico** — testi a 8 s, vecchia ROSSA col messaggio identico al censimento, nuova VERDE. ⚠️ **La strada «si aspetta la prossima rossa della CI» è definitivamente chiusa:** il difetto si compra, si nomina e si falsifica in un turno. *Era:* ⚠️ **la famiglia di asserzioni che corre contro timer corti**, e che rende la CI inaffidabile. *L'ipotesi del tetto è già stata smontata da una misura: 11–15 ms contro 500 di margine.* Il primo passo è **misurare sul runner**, non alzare numeri ⚠️ **2026-09-24 — LA CONDIZIONE DI QUESTA SCHEDA È CADUTA: IL DIFETTO SI RIPRODUCE IN LOCALE, A COMANDO.** Qui era scritto «si aspetta la prossima rossa col log già strumentato», frequenza **1 su 30** — cioè aspettare un evento raro su una macchina che non si può interrogare. **Misurato facendo F.2b: lanciando 21 file di test TUTTI INSIEME su 4 CPU — cinque volte il parallelismo della suite — cade una o due asserzioni per giro, DIVERSE ogni volta.** Tre giri: ① `test_dialogo_extra [Tocco]` ×2 · ② `test_new_features [A]` ×2 e `test_batch4 [T2]` ×2 · ③ di nuovo solo `[Tocco]`. *È alla lettera la frase di questa scheda — «un'asserzione a caso per corsa, e ogni volta una diversa» — riprodotta nel container.* ⚠️ **E il giro di CONTROLLO senza F.2b dà lo stesso quadro**, quindi la causa non è il passo: è la contesa. **Un difetto che si riproduce a comando smette di essere raro**, e il primo passo non è più aspettare: è strumentare sotto stress e leggere chi cade e perché. ⚠️ **2026-09-24, MISURA A CODICE FERMO — E HA TROVATO UNA CONTRADDIZIONE DENTRO `app/mappa.js`, FRA DUE SUOI COMMENTI.** Il commento a `aggiungiGruppiMagazzino` (riga ~1083) dice, con il caso che l'ha insegnata: *«un `renderConfigPanel()` completo azzera il `<details>` che l'utente aveva appena aperto»*, e per questo i due gruppi del magazzino si **aggiungono**. **Ma alla riga 573, quando arriva l'EPISODIO, il codice chiama `openConfigPanel()` — che alla riga 691 chiama `renderConfigPanel()`, cioè `innerHTML = ''` e ridisegna tutto.** *Il commento lì accanto dice «è la stessa forma di `aggiungiGruppiMagazzino`», e fa l'opposto.* ⚠️ **QUESTO È UN FATTO, LETTO NEL CODICE. Che sia la causa del rosso di `test_tabelle_personalizzazione [D]` NON è stabilito**, e va scritto separato: due sonde (ritardo sul magazzino fino a 3 s, poi sull'episodio) **non lo riproducono**, e la seconda dice perché — `intercettati: 0`, cioè in quello scenario l'episodio non viene mai chiesto. *La strada c'è nel codice; la prova che sia quella manca.* ✅ **E SPIEGA INVECE, senza sonda, il rosso di `test_config_estratto [D] → 0`:** `renderStoryCardsExplanationStatsPanel` esce subito su `riquadroSenzaEpisodio(el)` — **finché l'episodio non c'è, quel riquadro NON ha righe.** Il punto di approdo che l'asserzione non legge (regola 44) è quindi *«il riquadro non è più nello stato senza-episodio»*, non *«ci sono tre righe»*. |
| **F.2** · **F.4** | ⚠️ **MISURATI IL 2026-09-24, E SONO INTRECCIATI: F.4 VA PRIMA.** **① Il numero era invecchiato: i finti senza `speaking` sono VENTUNO, non diciassette.** **② E `speaking` non è un dettaglio del mock: gate il BLOCCO ASCOLTO.** `app/audio.js:140` → `staParlando() { return !!(synth && synth.speaking); }`, e la **prima riga** del listener globale è `if (!staParlando()) return;` — *quindi in quei ventuno file la Regola Azione Critica (regola 16) **non gira mai**, e nemmeno il ramo di `fcFlip` in Flash Card. Non è «il mock semplifica»: è che l'app gira con un meccanismo condiviso spento.* **③ NON ESISTE UN MOCK CONDIVISO: 38 file con un `mockInit` proprio, 27 VARIANTI DISTINTE** (la più diffusa in 4 file, la più grande 3775 caratteri in `test_comportamento_audio`). *Fare F.2 adesso vuol dire scrivere ventuno volte la stessa cosa in ventuno dialetti, e consegnare a F.4 un problema più grosso di quello che ha oggi.* **④ Un caso singolo che pesa da solo: `test_comportamento_audio.js` LEGGE `.speaking` da un finto che non lo dichiara** — il file che si chiama «comportamento audio» chiede una proprietà che non esiste, quindi legge sempre `undefined`: **vera per costruzione.** ✅ **DECISO il 2026-09-24: PRIMA F.4 (un `mockInit` condiviso), POI F.2** — così `speaking` vero si aggiunge **in un posto solo** e il rosso che nasce è leggibile. ⚠️ **AVANZAMENTO F.4 — 2026-09-24, E I NUMERI DI QUESTA SCHEDA ERANO SBAGLIATI.** Non «38 file, 27 varianti, 21 senza `speaking`»: **45 file, 28 varianti, 17 senza `speaking`**. *Il conto vecchio cercava la variabile `mockInit`, e **sei file il finto ce l'hanno con un altro nome** (`mockVoce`, `mockSilenzio`, `finta`): non erano mai stati contati. Il comando che lo rifà è `grep -l "defineProperty(window, 'speechSynthesis'" tests/test_*.js | wc -l` — regola 37: un numero che dipende dal nome della variabile misura la variabile.* ✅ **F.4 ② — `bootAsUser` — CHIUSO IL 2026-09-25: 25 file su 32.** Il numero «16 varianti» che avevo dato era **il conto byte per byte**, e contava come diverse due copie separate solo dagli spazi: normalizzando sono **20 corpi**, e guardando il solo **scheletro** (fuori dal `localStorage`) sono **25 file identici su 32**. *Un conto che dipende dagli spazi misura gli spazi — regola 37.* **Il guadagno non è togliere le copie: è che portavano dentro `'gate'` inciso in ventisette file e la lista dei moduli in ventotto.** ⚠️ **UN ROSSO, E NON ERA MIO, ma la prova non è il rilancio verde:** `test_batch19 [SR Task3-adj] «Managed to observe a correct-answer tap within retries»` è caduto 1 giro su 3 — *ed è la famiglia che il file stesso documenta in testa: «tocca la prima opzione e spera», rosso 2 giri su 3 il 2026-09-08*. **La prova che il passo è innocente è una SONDA: il `localStorage` dopo il boot nuovo ha esattamente le 18 chiavi del boot vecchio**, misurate, non argomentate. *Un rilancio verde non avrebbe provato niente (regola 19).* **Aggiunge un'osservazione a 1.18: le asserzioni della famiglia osservate salgono a quattro.** **I sette che restano**, col motivo in `tests/boot.js`: `test_sequenze` (azzera il `localStorage` e ricarica — altro mestiere), `test_outcome_step_ids` (ne ha **due**), e cinque che **aspettano un selettore invece di un tempo** — `story_modules`, `mastery_al_gesto`, `blocco_ascolto`, `avviso_microfono`, `sblocco_sequenziale`: *portarli allo scheletro sarebbe un passo **indietro**.*

✅ **F.4 È CHIUSO IL 2026-09-25: 42 FILE SU 45, E I TRE CHE RESTANO HANNO CIASCUNO IL SUO MOTIVO SCRITTO.** ⚠️ **E il residuo era più grande di come l'avevo stimato: avevo detto «una sola opzione nuova nel nucleo», la misura ne ha chieste quattro** — `spiaToni`, `catturaAvvisi`, `componi` e la **quinta forma** del riconoscimento. *Regola 45 ①: la stima invecchia, la misura no.* **E la spia dei toni non era in 7 file: era in 13.** ⚠️ **I TRE NON CONVERTITI, e nessuno dei tre è un debito da smarcare:** ① **`test_attese_condivise` non finge il browser per l'app** — ha una pagina sintetica (`setContent`) per provare `attese.js`, e chiama `parlaPerMs()` a mano: *non è convertibile, punto*; ② **`test_comportamento_audio` è uno strumento di misura suo**, non un finto: registra ogni evento in `window.__audio.eventi` e tiene una registrazione aperta apposta per far vedere il guasto; ③ **`test_batch13` chiederebbe DUE cambi al nucleo**, e il secondo non è gratis: la leva `__vcNeverResult` (facile) **e la guardia `_stopped` su `suStop`**, che cambierebbe comportamento a **dieci** file già convertiti. *È un passo suo, con il suo rosso possibile — non una coda.* **Cosa è nato:** `spiaToni` (13 copie), `catturaAvvisi` (3 copie identiche di `console.warn`), `componi` (mette più pezzi in un `addInitScript` solo, così i **sessanta** punti di chiamata non si toccano), la forma `'continuo'` (l'unica in cui arriva qualcosa **mentre** si parla — viene da `batch14`, ed è quella che prova il timeout di silenzio che scattava a metà frase) e il gancio `__annotaTono`. ✅ **E il passo ha trovato tre cose morte**, vedi `correzioni.md`. *Era:* **Convertiti 35 file su 45 — 21 nei giri precedenti, 14 il 2026-09-25 (coda di F.4).** ⚠️ **I 14 sono quelli che NON chiedono `AudioContext` né le leve di Voice Coach**, e il nucleo ha dovuto crescere di **due sole cose**, tutte e due misurate e non scelte: **① `ritardoCancelMs`** — tre finti ritardavano apposta la chiusura di `cancel()` (15 ms in `test_batch18`, 30 in `test_batch5`, 5 in `test_batch6`) e il loro commento dice perché: *«dare al callback vecchio ogni occasione di sbagliare»*. Portarli a zero avrebbe indebolito le loro asserzioni **senza che nessun rosso lo dicesse** — la regola 44 al rovescio. **② `window.__detti`** — la lista di cosa è stato detto, che tre file si scrivevano con tre nomi diversi. ✅ **E LA CODA HA TROVATO UN DIFETTO NEL NUCLEO, col rosso:** `cancel()` rimandava al macrotask successivo **anche lo stato**, non solo l'evento, quindi `speaking` restava `true` dopo l'interruzione. *`test_batch5 [Job1]` legge `speaking` nella stessa chiamata sincrona del click, e aveva ragione lui: in un motore vero `cancel()` interrompe subito.* Corretto nel nucleo, non nel file. **Restano 10 file, contati e non stimati**: **sette** con `AudioContext` (`batch10`, `batch11`, `batch12`, `batch14`, `batch15`, `batch16`, `batch17`), `test_batch13` con la sua leva `__vcNeverResult`, `test_comportamento_audio` che è uno strumento di misura suo, e `test_attese_condivise` che **non ha un finto dell'app**: ha una pagina sintetica per provare `attese.js`, quindi **non è convertibile e non è un debito**. *Il vecchio conto «restano 24» era giusto e il vecchio: sono 24 meno i 14 di oggi.* *Era:* **Convertiti finora 21 file su 45**: i 4 del gruppo 1, poi i 17 di questo giro. **Restano 24, e sono ESATTAMENTE quelli che `speaking` ce l'hanno già** — cioè quelli che F.2 deve toccare comunque. ⚠️ **DECISO: `test_batch11` e `test_batch12` NON sono stati convertiti benché siano un gruppo**, ed è una scelta da dichiarare: il loro finto non è solo un finto, è uno **strumento di misura** (cattura di `console.warn`, `speaking` con stato vero, spia su `AudioContext` per i toni). Convertirli oggi vorrebbe dire aggiungere al mock condiviso tre pezzi opzionali `speaking` / `toni` / `avvisi` — *e `speaking` è proprio quello che F.2 renderà il default fra un passo, cancellando l'opzione appena scritta.* **Vanno con F.2, non prima.** ⚠️ **E una cosa trovata convertendo, tolta nello stesso commit:** il finto di `test_batch2` contava gli avvii del riconoscimento in `window.__recognitionStarted` — **un contatore che nessuno legge**, in tutto il repository. *È la forma della misura che non misura vista dall'altro lato: non un numero sbagliato, un numero che non viene mai guardato.* ⚠️ **F.2 SI È SPEZZATO IN TRE, E NON PER PRUDENZA: `speaking` vero e `cancel()` che manda `onend` sono DUE cause, e un rosso a due cause non è una misura.** **F.2a** — il nucleo condiviso dichiara `speaking` (fatto il 2026-09-24). **F.2b** — `cancel()` manda `onend` **in modo asincrono**, com'è nel browser vero: *è il fatto su cui l'app ha costruito l'epoca*, quindi è il passo che può far emergere il sanguinamento dell'audio fra moduli. **F.2c** — `paused` vero. ⚠️ **PRIMO ROSSO DI F.2a, E NON È DETERMINISTICO: `test_dialogo_extra` → `[Regression] Mod1 alza la bolla che sta parlando`, 1 giro su 10, e il giro rosso è quello con VENTUNO FILE IN PARALLELO.** *Cioè la finestra si apre sotto carico: è la famiglia della regola 19, e il carico del container in parallelo è l'unica cosa che qui somiglia al runner.* ⚠️ **DIAGNOSI RITIRATA IL 2026-09-24: NE HO SCRITTE DUE E LA MISURA LE HA SMONTATE ENTRAMBE.** *(a)* «il tocco arriva mentre la prima battuta suona» — **falsa**: prima del click `speaking` è `false`, `dgStartExercise` non suona niente e la bolla parla **solo perché la si clicca**. *(b)* «`is-active` vive 25 ms e la sonda salta la finestra» — **falsa anche lei**: con la finestra a **1 ms** la forma vecchia resta verde, e resta verde anche con otto processi a saturare le CPU. ⚠️ **LA CAUSA RESTA IGNOTA**, ed è scritto così apposta invece che con una terza storia: osservata **una volta**, nel giro con ventuno file in parallelo, non riprodotta in 12 giri isolati, nella suite completa, a 1 ms né sotto contesa. ⚠️ **E NON È UN ROSSO DI F.2a:** il commento di quell'asserzione racconta che era già caduta lo stesso giorno su un albero in cui il Dialogo non era stato toccato. **L'asserzione è stata comunque riscritta** — click e lettura nella stessa chiamata sincrona (regola 19) — *non come correzione del rosso, ma perché toglie del tutto la dimensione tempo; e legge `is-active` e `speaking` insieme, che è possibile solo grazie a F.2a.* | ⚠️ **Resta aperto: se quel rosso torna, si è davanti a una causa mai identificata** — e adesso l'asserzione non ha più un tempo su cui incolparlo ✅ **F.2c FATTO il 2026-09-24: `paused` è vero.** Non un'etichetta: `pause()` ferma il timer e si ricorda quanto mancava, `resume()` riparte da lì. *Un `paused` che cambia solo un booleano direbbe «sono in pausa» mentre l'audio finisce da solo alla sua ora — la stessa forma del finto che semplifica troppo (regola 19).* ⚠️ **ZERO ROSSI, e il motivo è una TROVATA che vale più del passo: `pausaLaVoce()` e `riprendiLaVoce()` NON POSSONO MAI FARE NIENTE.** Misurato: il loro unico chiamante è `dgTogglePause` (`app/dialogo.js:589`), dietro il pulsante `dg-pause-btn`, e `dgUpdateToolbarButtonState` lo tiene **spento** con `pauseBtn.disabled = !isCountingDown && !dgPaused` — cioè **acceso solo mentre scorre il countdown, che parte quando l'audio è FINITO**. Ma `pausaLaVoce()` agisce solo `if (staParlando())`. *Il pulsante è spento esattamente quando quelle due funzioni avrebbero qualcosa da fare.* **Trovato e NON corretto:** è la stessa famiglia di F.5 — codice che non può agire — ma più sottile, perché **le funzioni vengono chiamate**, e non fanno niente. Toglierle o cambiare la regola del pulsante è una decisione di chi guida il progetto: *forse il pulsante DOVREBBE poter fermare la voce, e allora il difetto è il `disabled`, non le funzioni.* ✅ **LA PAUSA DELLA VOCE È FATTA, IL 2026-09-25 — E LA MIA PRIMA PROPOSTA ERA SBAGLIATA.** *Avevo proposto «il difetto è il `disabled`»: il codice dice il contrario in tre punti — «"Pausa" pausing mid-AUDIO **used to hang the whole dialogue** (`synth.pause()` is unreliable mid-utterance)», Job 2, 3° collaudo. **Quel `disabled` era la cura di un difetto trovato**, e toglierlo lo avrebbe rimesso in piedi.* **La strada presa non tocca `synth.pause()`:** alla pausa la battuta si **interrompe** (`cancel()`, onesto dal passo F.2b), alla ripresa **si risuona da capo**. ⚠️ **Due cose misurate, non dedotte, durante il passo:** ① *`cancel()` fa arrivare `onend`, e su questo profilo `onend` fa partire il countdown della battuta dopo* — **senza una guardia la pausa avrebbe fatto AVANZARE il dialogo**, il contrario di quel che chiede; ② dopo l'interruzione la bolla conservava la classe `speaking`, e `toggleSpeak` la legge come «questo sta già parlando»: **la ripresa usciva senza risuonare niente**, col pulsante che tornava a dire «Pausa» e silenzio. *Visto con una sonda, non dedotto.* ⚠️ **E IL PASSO HA TROVATO UN'ASSERZIONE VERDE PER IL MOTIVO SBAGLIATO:** `test_batch16 [Job2] «Pausa is disabled while a line's AUDIO is actively speaking»` leggeva dopo 400 ms fissi, e **misurato in quell'istante: `speaking: false`, zero bolle attive, zero countdown — si era ancora dentro il 3-2-1 di avvio.** Il pulsante era spento perché non succedeva niente. *Sarebbe rimasta verde anche il giorno in cui il comportamento fosse cambiato — e infatti lo è rimasta.* Riscritta insieme alla sua vicina, che ci costruiva sopra. ⚠️ **`pausaLaVoce` e `riprendiLaVoce` adesso NON le chiama più nessuno** (prima venivano chiamate e non facevano niente): **trovate e non tolte** — cancellarle è una decisione sullo strato audio, non sul Dialogo. |
