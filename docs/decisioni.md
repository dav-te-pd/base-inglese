...
**La fase 2 è stata fatta:** ventidue passi su profilo nuovo, su Pages, il 2026-09-09.
...
**Il primo dei cinque, C.3, è fatto e verificato.** Restano C.5, C.4 e C.2, in
quest'ordine. Non si comincia senza il via di chi guida il progetto.
...
| 2026-09-09 | **Una rinomina si verifica su TUTTE le forme del nome, non su quelle a cui si è pensato.** Almeno queste, e si contano prima di cominciare: `camelCase` (`speedRound`), **kebab** (`speed-round`), **minuscolo attaccato** (`speedround`, i prefissi delle chiavi mastery), **CON LO SPAZIO** (`Speed Round`, `SPEED ROUND`, `speed round`), e il nome **dentro un identificatore più lungo** con la maiuscola interna (`openSpeedRound`, che una ricerca sul nome nudo non prende), e — per i percorsi — **il percorso ASSEMBLATO A PEZZI**: `repoPath('data', 'it', nome)`, `['data', 'it', …]`. Lì la stringa `data/it/` **non esiste mai per intero**, quindi nessuna ricerca testuale la trova. | **Lo stesso difetto è capitato DUE VOLTE nello stesso giorno**, ed è la ragione per cui questa riga esiste. Nei passi 3 e 4 la verifica cercava `quick-match` col trattino: le **185 occorrenze con lo spazio** le sono passate davanti, e il passo è stato dichiarato fatto. Nel passo 5, la stessa forma è stata trovata **prima** di dichiarare — perché quella volta avevo misurato invece di stimare. La differenza fra le due volte non è stata l'attenzione: è stato il conteggio fatto prima. | **La quinta forma è stata aggiunta il 2026-09-09, dopo che il passo 6 l'ha trovata sul campo:** sedici file su quarantadue rossi, tutti con `ENOENT data/it/inglese-it-gate.json` — il nome del file aggiornato, la cartella no. **La regola aveva ragione sul principio e il suo elenco era incompleto**, il che è esattamente ciò che dice di sé: le forme sono quelle che esistono, non quelle a cui si è pensato. Ce ne sarà una sesta.<br><br>**Vale da subito, e il passo 6 è quello che la mette più alla prova**: `gate`, `episode-1`, `"Episodio 1"`, `docs/inglese/it/`, `data/inglese/it/`, `a1-episodio1-inglese`. Se questa riga non ci fosse, la terza volta capiterebbe lì. |
