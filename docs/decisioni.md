...
| **5** | `se* → storyCards*`. **Una sessione sola.**<br><br>🛑 **BLOCCATO il 2026-09-09, prima di toccare il codice: la fonte si contraddice.** `docs/it/struttura-corso.md` dice due cose incompatibili — la **tabella dei nomi** dice `se*` → `storyCards*`, ma la sezione «Cosa segue il nome, e cosa no», punto 3, elenca *«`qm-`, `sr-`, `fc*`, e **`se*` stesso nei nomi di funzione**. Queste NON seguono»*. Le due letture differiscono di **~290 occorrenze** e cambiano se questo passo sia o no quello pericoloso. La frase del punto 3 l'ho scritta io, ed è mia l'incoerenza. **Non si comincia finché chi guida il progetto non decide quale delle due vale.**<br><br>⚠️⚠️ **SE LO STATO DI QUESTA RIGA È ◐ E TU NON STAI LAVORANDO ADESSO, IL PASSO È INCOMPLETO.**<br>**NON FIDARTI DEL VERDE:** un `se-` mancato in una regola CSS non fa fallire nessun test — `test_hidden_guard.js` costruisce gli elementi *a partire dalle* regole, quindi una regola orfana passa. **RIFALLO DA CAPO:** `git checkout main && git checkout -B claude/verifica-in-corso origin/main`, e ricomincia dallo strato 1. Non continuare da dove sembra essere arrivato.<br><br>**COSA SIGNIFICA UN COMMIT SU QUESTO PASSO, e non è ovvio.** Un commit qui **non prova niente**. I sei strati vanno in **un commit solo, alla fine**: se ne trovi uno mentre il marcatore è ◐, quello è **lavoro salvato per non perderlo** — perché niente deve vivere solo nel container (regola 22) — **non lavoro finito**. Il commit esiste per la stessa ragione per cui esiste questa riga: perché quello che c'è si veda, non perché sia buono. **Il marcatore, non il commit, dice se il passo è fatto.**<br><br>**Perché rifare e non continuare** — è la domanda giusta e la risposta è una sola: la correttezza di questo passo si stabilisce **leggendo il diff**, non lanciando la suite. Chi non l'ha scritto deve leggerlo tutto, e leggerlo costa quanto rifarlo. E un albero a metà rende **ambigua la verifica per sottrazione**: un `se-` ancora presente può voler dire «non ancora fatto» oppure «fatto male», e non c'è modo di distinguerli. Da una base pulita, invece, «il nome vecchio è sparito» è una prova.<br><br>**I sei strati, con i numeri veri (misurati il 2026-09-09):**<br>1. identificatori JS `se[A-Z]*` — **116** in `index.html`, 5 nei test (24 nomi distinti)<br>2. classi e id CSS `se-*` — **42** + 27 nei test<br>3. id DOM `speak-easy-*` — **54** + 41 nei test<br>4. i due namespace del `localStorage` (`seDeclarations:`, `seExplanationStats:`) — 9, dentro lo strato 1<br>5. `speakEasy` / `SpeakEasy` — **29** + 36 nei test + **1 chiave VIVA in `data/it/messaggi-feedback.json`** (`speakEasyCompleteMessages`, letta a `index.html:7477`)<br>6. **«Speak Easy» in prosa — 24 + 34 + 6.** ⚠️ **Questo strato NON è un `sed`:** molte di quelle righe raccontano la storia del nome tolto («il modulo non esiste più», «diceva Speak Easy a entrambi»), e cambiarle le renderebbe false. Vanno lette una per una.<br><br>**~420 occorrenze in tutto**, contro le ~283 stimate prima di misurare. | ☐ | **sì**, ma solo DOPO che è finito per intero |
