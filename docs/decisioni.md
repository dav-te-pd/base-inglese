...
**Fase 1: i passi 1, 2, 3, 4 e 5 sono fatti e verificati, tutti in `main`.**

Resta il **passo 6**, gli episodi: gli id, lo spostamento delle cartelle sotto
`docs/inglese/it/` e `data/inglese/it/`, i nomi dei file, `messaggi-feedback.json`
con il percorso in una costante, le regole 4 e 26 di `CLAUDE.md` — **più la
tabella «I nomi in codice» di `struttura-corso.md` da riscrivere al passato,
tutta insieme**, e la regola sulla lista attività. Un passo per volta, con la fermata in mezzo, e non
...
| 2026-09-09 | **Una rinomina si verifica su TUTTE le forme del nome, non su quelle a cui si è pensato.** Almeno queste, e si contano prima di cominciare: `camelCase` (`speedRound`), **kebab** (`speed-round`), **minuscolo attaccato** (`speedround`, i prefissi delle chiavi mastery), **CON LO SPAZIO** (`Speed Round`, `SPEED ROUND`, `speed round`), e il nome **dentro un identificatore più lungo** con la maiuscola interna (`openSpeedRound`, che una ricerca sul nome nudo non prende). | **Lo stesso difetto è capitato DUE VOLTE nello stesso giorno**, ed è la ragione per cui questa riga esiste. Nei passi 3 e 4 la verifica cercava `quick-match` col trattino: le **185 occorrenze con lo spazio** le sono passate davanti, e il passo è stato dichiarato fatto. Nel passo 5, la stessa forma è stata trovata **prima** di dichiarare — perché quella volta avevo misurato invece di stimare. La differenza fra le due volte non è stata l'attenzione: è stato il conteggio fatto prima. | **Vale da subito, e il passo 6 è quello che la mette più alla prova**: `episode1`, `episode-1`, `"Episodio 1"`, `docs/it/`, `data/it/`, `a1-episodio1-inglese`. Se questa riga non ci fosse, la terza volta capiterebbe lì. |
