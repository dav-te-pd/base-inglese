...
| **②** | **La riga in `CLAUDE.md` sul test che guida un caso solo**, con le parole di chi guida il progetto: *«UN TEST CHE DICHIARA DI GUIDARE UN CASO SOLO NON BASTA A PROTEGGERE UNA RIGA CHE VALE PER TUTTI. Se la riga è condivisa, il test deve toccare almeno il caso più DIVERSO — non il più comodo.»* Con accanto l'esempio del giorno: Personalizza è **l'unico di sedici moduli senza `dataFile`**, e il test appena scritto guidava Voice Coach, cioè il caso più simile. | ☐ **il prossimo** |
