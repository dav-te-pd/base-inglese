...
| Data | Cosa | Perché | Quando si esegue |
|---|---|---|---|
| 2026-09-07 | `test_batch19.js`, due cose rimaste: **(1)** il `.catch(() => {})` sul click di `#sr-ready-btn` sopprime il fallimento del click — va sostituito con un `waitForSelector('#sr-ready-btn')` prima, così se il pulsante non c'è il test fallisce lì e lo dice. **(2)** la `waitForTimeout(300)` subito dopo è ridondante rispetto alla riga che aspetta già lo stato vero — via, e il test è anche più veloce. | **La diagnosi registrata qui era sbagliata, ed è la cosa da ricordare.** Diceva che il click soppresso era «probabile causa vera del rosso in CI» e che senza (1) il resto non risolveva niente. Il rosso vero, riprodotto il 2026-09-07 con tre giri locali (due verdi e un rosso sullo stesso codice), aveva **un'altra causa**: i cicli provavano venti giri su nove domande e finivano per cliccare in una schermata senza opzioni. E cadeva ogni volta su una **riga diversa** — 139 in una corsa, 244 in un'altra — che è il segno che il difetto era condiviso, non locale. Una diagnosi scritta e mai verificata è un'istruzione sbagliata che aspetta qualcuno. Corretta in `correzioni.md`. | **Restano, e valgono lo stesso**: sono due rischi veri (un click che fallisce in silenzio, un'attesa fissa che finge di servire), ma non erano la causa di quel rosso. **Alla prossima apertura di quel file.** |
