...
| **24** | `docs/inglese/it/componenti-ui.md` si riempie **nello stesso commit** di ogni estrazione. Criterio: un pezzo ci sta **se e solo se** è usato da più di un modulo — condizione verificabile, non prosa. Il file **nasce con la prima estrazione**, non prima: un file vuoto in attesa è un invito a riempirlo di intenzioni. ⚠️ Una riga rimandata è una riga scritta dopo guardando il risultato, cioè un censimento invece di una decisione registrata. | ☐ | **sì** |
