...
| **1.7** | **`app/magazzino.js` — l'unico file che sa DOVE SI SALVANO I PROGRESSI** | ⚠️ **RINOMINATO E RIMISURATO il 2026-09-20, e il nome vecchio era il problema.** Si chiamava «l'astrazione della **fonte**» e diceva «Supabase in UN file invece che in **101 punti**». Misurato: i punti sono **27**, e **metà del passo è già fatta** — i cinque `fetch` dei file di dati passano tutti da `percorsoEdizione` (passi 1.9 e 1.11), cioè **sono già un punto solo**. *Il nome mandava a guardare proprio la metà fatta.* **Quello che resta sono 22 accessi a `localStorage` su DODICI chiavi**, in sei file: `progressi.js` 8 · `mappa.js` 5 · `identita.js` 4 · `avvio.js` 2 · `dati.js` 2 · `apertura.js` 1. **Non sono i dati che arrivano: sono i progressi che si salvano** — che è la cosa chiesta esplicitamente («i progressi dello studente vivranno nel server, nel browser non va nulla»). ⚠️ **È il passo che decide quanto costerà Supabase**, e adesso ha un perimetro misurato invece di un numero ereditato |
