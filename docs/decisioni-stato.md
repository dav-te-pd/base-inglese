...
| | Passo | Perché adesso |
|---|---|---|
| **1.3** | **Passo 18 — le stringhe del markup** | ⚠️ **RICONTATE E FATTE IN PARTE il 2026-09-20 (1.3a): 95 occorrenze in `index.html`, 47 distinte; 54 spostate.** Restano 41, e **nessuna per dimenticanza**: 9 di onboarding/home (devono funzionare quando niente funziona), 7 del Pannello Admin (strumento, non studente), 15 **sovrascritte a runtime** (segnaposto), 6 della **mappa** e 4 varie. ✅ **1.3b FATTO il 2026-09-21:** la mappa aspetta i suoi testi, con la sua schermata d'errore come ogni modulo. La guardia costa solo al pulsante di casa — gli altri nove punti di chiamata sono un «← Mappa» dentro un modulo, dove i testi ci sono per forza. ✅ **1.3d FATTO il 2026-09-21:** `moduleLabels` — **trenta testi che lo studente legge**, i nomi e i sottotitoli dei quindici moduli — esce da `app/config.js` e va in `struttura-corso.json`, cioè nell'edizione. ⚠️ **Il censimento di 1.3c non le aveva viste**, pur cercando proprio quelle stringhe: guardava dove una stringa **arriva allo schermo**, e queste sono valori di configurazione letti per chiave. *Una misura giusta può avere un punto cieco, e il punto cieco somiglia a un elenco completo.* ✅ **1.3c FATTO il 2026-09-20:** censimento vero su `app/*.js`, guardando **dove la stringa arriva allo schermo** e non il letterale in sé — **28 punti**, di cui **nove spostate** in cinque file. `Pausa` stava in due posti (file dei testi + codice del toggle) e le due potevano divergere; `Risposta corretta: ` era duplicata in `match.js` e `speedmatch.js`. Il resto resta con motivo: Pannello Admin, `console.warn`, le due frasi di ultima istanza, e la home. ✅ **E le due cose trovate e non corrette sono CHIUSE il 2026-09-21**, insieme a 1.3b: le tre copie di ripiego in `app/personalizza.js` e il badge di Repeat Aloud senza `id`. *Erano registrate con «Condizione: 1.3c», e la condizione era scaduta il giorno prima — vale la pena dirlo: è il difetto che questo progetto ha già pagato, una condizione che matura e nessuno se ne accorge.* |
