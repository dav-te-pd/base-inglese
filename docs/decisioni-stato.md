...
| **F.7** | **I DUE LETTORI DEL RIQUADRO «Numeri attesi», scritti due volte.** `numeriAttesiDallaFonte()` in `test_story_modules.js` e `numeriAttesi()` in `test_episodio2.js` fanno la **stessa cosa** su due episodi diversi. ⚠️ **Non è un sospetto: si sono rotti INSIEME il 2026-09-21**, quando il riquadro è passato da citazione `>` a paragrafo e da *«voci nel grado A»* a *«voci in A»* — 99 asserzioni ferme su due file, per una riga sola di forma. *Adesso che `REGISTRO-EPISODI_004` dichiara le tredici sezioni **uguali per ogni episodio**, il formato è uno: un lettore solo lo legge per tutti, e il terzo episodio non rompe un terzo test.* | **quando nasce il TERZO episodio** — prima no: oggi il lettore condiviso costerebbe una suite in più su un rosso da chiudere subito, e con due copie il prezzo del rinvio è noto e limitato |
