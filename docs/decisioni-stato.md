...
|---|---|---|
| **1.3** stringhe del markup | **CODICE** | ✅ **FATTO IL 2026-09-26 per la parte che si poteva fare.** ⚠️ **Il numero qui accanto era invecchiato di molto:** misurato il 2026-09-26, **33 occorrenze e 28 distinte**, non 101 e 48 — i 65 `data-testo` dei giorni scorsi avevano già fatto la maggior parte. **Cinque candidati veri, tutti spostati**, e tre erano **duplicati di chiavi già esistenti**. **Resta `1.3b`**, che è una decisione sul comportamento e non uno spostamento: la mappa non aspetta i testi, quindi le sue stringhe restano nel markup. *Era:* 101 occorrenze in `index.html`, **48 distinte**; 64 occorrenze sono **15 stringhe ripetute** (`Spiegazione` ×19, `Help` ×9, `← Mappa` ×7). **Almeno 24 delle 48 NON si spostano**: 8 di onboarding/home (decisione presa — devono funzionare quando niente funziona), 8 del Pannello Admin (strumento, non studente), 8 nomi di moduli **sovrascritti a runtime** da `CONFIG.moduleLabels`, cioè segnaposto |
...
| **F.5** | **`episodeFinalOutcomeCase`** e **`buildTargetTokens`**: nel codice, **nessun chiamante** | ✅ **DECISO il 2026-09-21: si cancellano.** Il disegno resta in `decisioni-storico.md` e si riscrive quando nascerà il Modulo Finale. *Codice che nessuno chiama è una bugia su cosa fa l'app* ✅ **FATTO il 2026-09-24.** Cancellate: `episodeFinalOutcomeCase` (30 righe con il suo commento, `app/mappa.js`) e `buildTargetTokens` (38 righe, `app/ui-condivisa.js`). *Il disegno resta in `decisioni-storico.md` e si riscriverà quando il Modulo Finale nascerà davvero.* ⚠️ **E LA CANCELLAZIONE HA LASCIATO UN ORFANO, TROVATO E NON TOLTO: `segments` NEI DESCRITTORI DEGLI EPISODI ORA NON HA NESSUN LETTORE.** Il commento in `app/catalogo.js` diceva *«lo legge solo `buildTargetTokens`»*: quella funzione non c'è più, e con lei l'ultimo lettore. **Non è stato tolto nello stesso passo, di proposito:** toglierlo cambia **la forma dell'episodio**, cioè un dato, e F.5 era deciso su due funzioni. *Una cancellazione decisa per A che si porta dietro B non è la decisione che è stata presa.* ⚠️ **Tre frasi rese false dal commit, corrette nello stesso commit** (regola 33): il commento di `segments`, quello che citava `buildTargetTokens` fra le diagnostiche d'autore (`app/catalogo.js`), e l'intestazione del blocco di `test_batch11` che nominava `episodeFinalOutcomeCase`. *Quel blocco resta: protegge la forma di `loadModuleOutcomes`, che serve comunque.* ✅ **E L'ORFANO È CHIUSO IL 2026-09-25: `segments` NON C'È PIÙ.** *Double-check chiesto da chi guida il progetto prima di cancellare.* **La prova che si poteva togliere non è un grep, è un fatto: `aircraft-door` non l'ha mai avuto** — è sempre stato `{}` — **e l'app ci funziona sopra da quando esiste.** *«Un episodio senza `segments`» non era un caso da provare: era già il caso normale, metà del catalogo.* Le forme cercate (regola 41), zero compresi: `.segments` come chiave raggiunta **0**, dentro un identificatore più lungo **0**, al singolare **0**, nei file di `data/` **0**. *Il contenuto della frase non si perde: vive nel grado C del file episodio, che è la fonte vera — lì era una **seconda copia**, scomposta per una vista che non esiste più.* |
