...
| | Cosa | Condizione |
|---|---|---|
| **1.13** | **Le sequenze di EPISODI.** Stesso identico sistema dei moduli, un livello sopra: `episodeSequences` nel file di struttura, l'**edizione** dichiara quale usa, e il Pannello Admin le sceglie e le riordina con gli stessi due menu. ⚠️ **Lo studente non sceglie mai** — è chi guida il progetto che organizza. **Oggi l'ordine degli episodi non è un dato:** è l'ordine in cui sono scritte le chiavi dentro `episodes`, e nessuna riga di codice lo dichiara ⚠️ **E PORTA CON SE' LA TERZA VARIANTE DELLO SBLOCCO SEQUENZIALE, approvata in chat il 2026-09-21 e scritta qui il 2026-09-21 perche' fino ad oggi viveva solo nella conversazione:** oltre a quelle **per ascolto** (Ripeti a Tempo) e **per dichiarazione** (Why We Say It) della regola 30, nasce quella **per completamento** — *l'ULTIMA POSIZIONE della sequenza sblocca l'episodio successivo.* ⚠️ **L'ultima POSIZIONE, non un modulo:** gli episodi grammaticali avranno meno moduli, quindi legare lo sblocco a un modulo per nome romperebbe ogni episodio che non ce l'ha. *«Non deve essere collegato ad un modulo lo sblocco dell'episodio successivo, ma all'ultima posizione.»* **Quando la variante nascera', la regola 30 di `CLAUDE.md` va aggiornata** — oggi dice «due varianti». ⚠️ **MISURATO il 2026-09-22, e la superficie e' piccola: l'ordine degli episodi e' letto in DUE punti soli**, e il secondo non si trova leggendo il piano — ① il menu dell'interruttore admin (`renderEpisodeSwitchField`, da `Object.keys(BI.EPISODES)`) e ② **il ripiego all'avvio** (`app/mappa.js:491`): se `CONFIG.episodioCorrente` nomina un episodio che non esiste, apre `Object.keys(EPISODES)[0]`. *Un ripiego che dipende da «il primo» dipende gia' da un ordine che nessuno ha dichiarato.* **E tutto il resto esiste gia':** `EPISODES` porta TUTTI gli episodi coi moduli gia' risolti (`costruisciPassi`), e `loadModuleProgress(episodeId, userName)` prende l'id come argomento — il progresso di un episodio qualunque si legge in una riga. ✅ **DECISO il 2026-09-22: `CONFIG.episodioCorrente` SI SPOSTA NELL'EDIZIONE DENTRO QUESTO PASSO.** E' l'ultimo superstite della famiglia che il passo 1.11 ha portato nell'edizione — `gate` non esiste in un'edizione francese — e 1.13 apre gia' quei file. | **dopo la tappa ①**, e **prima di 1.12**: senza, il collaudo delle cinque edizioni prova metà cosa |
...
| **1.13-bis** | **LA VISTA EPISODI, e il fatto che la ridimensiona: IL GIRO DELLO STUDENTE HA QUATTRO PASSI E L'APP NE HA DUE.** Descritto da chi guida il progetto il 2026-09-22: *«accede al sito, fa il login, **sceglie il corso** (l'edizione inglese-it), clicca sul **primo episodio**, poi sul primo modulo»*. ✅ login = `view-onboarding` · ❌ **scelta del corso: non esiste** — `CONFIG.edizione` e' un valore globale che si cambia solo dal Pannello Admin · ❌ **scelta dell'episodio: non esiste** — `CONFIG.episodioCorrente` e' un valore, e `view-home` ha un pulsante solo · ✅ scelta del modulo = `view-map`. ⚠️ **`view-home` («Ciao X!» + «Inizia ‹Al gate›») e' il posto dove i due passi mancanti sono schiacciati in un pulsante fisso.** Porta con se' lo Sblocco Sequenziale **per completamento** (vedi 1.13) e `episodeStatus`, gemello esatto di `moduleStatus` — quattro righe. **La scelta del CORSO non sta qui: e' di 1.12**, e non si puo' provare finche' un'edizione e' una sola | **dopo 1.13**, e **codice + codice: mai insieme a lui** (regola 45) |
| **1.17** | ⚠️ **NESSUNA DELLE QUINDICI CHIAVI DEL MAGAZZINO PORTA L'EDIZIONE.** Nove sono `baseinglese:<cosa>:<episodeId>:<userName>` — moduli, esiti, mastery, personalizzazione, e le altre. **Un'edizione nasce COPIANDO la cartella** (regola 4), quindi un `gate` francese e un `gate` inglese sono lo stesso id: **scriverebbero nella stessa chiave.** Lo studente che fa `gate` in inglese e poi apre `gate` in francese lo troverebbe gia' mezzo fatto, **senza nessun errore.** *Oggi non e' un difetto: l'edizione e' una sola. Lo diventa il giorno della seconda.* ⚠️ **E pesa il doppio per una ragione dichiarata da chi guida il progetto: «i progressi dello studente vivranno nel server, non nel browser»** — la forma della chiave diventera' la chiave primaria di una tabella, e sistemarla prima costa una riga, dopo costa una migrazione | **PRIMA di 1.12**: la catena delle cinque edizioni e' esattamente il collaudo che ci sbatterebbe contro, e ci sbatterebbe **in silenzio** |
