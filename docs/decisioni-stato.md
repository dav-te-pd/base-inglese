...
| **1.18** | ⚠️ **IL TETTO DI 15s DI `attendiDomandaSuccessiva` IN `test_batch19.js`, e il runner ci sbatte contro.** Trovato il 2026-09-22 col passo 1.13: **CI rossa su una suite verde in locale**, una sola asserzione — *«[SR Task1] "Non lo so" resets to enabled on the next question»* — con lo stato alla resa `{contatore: "2 / 9", revealAperto: false, spento: true}`: **Speed Match non e' avanzato entro 15 secondi.** ⚠️ **NON e' di 1.13** — il diff non tocca Speed Match, i due commit precedenti erano verdi, e **il rilancio (uno solo, come ammette la regola) e' andato VERDE**. ⚠️ **E «flake» non e' una causa (regola 37): la causa ha un nome.** L'attesa e' scritta bene — e' una `waitForFunction` su cio' che il lavoro produce, non un tempo — ma **il suo TETTO e' un tempo**, e Speed Match e' l'unico quiz con un conto alla rovescia in mezzo: sul runner, piu' lento del container in modo sistematico (regola 19), la catena pausa + timer + avanzamento ci puo' stare dentro a fatica. *Il tetto non e' sbagliato: e' l'ultimo pezzo a tempo di un'attesa che per tutto il resto non lo e'.* | **quando si tocca Speed Match o la famiglia delle attese**. Prima no: alzare il tetto a occhio senza misurare quanto ci mette davvero sarebbe sostituire un numero scelto con un altro numero scelto |
