...
| | Cosa | Condizione |
|---|---|---|
| ~~**F.1**~~ | ✅ **CHIUSA LA ① il 2026-09-21** — `speechstart` collegato, il taglio non prende più chi sta parlando. ⚠️ **Resta APERTA la ②**, che non era nella riga originale e il censimento ha fatto emergere: **non esiste nessun timer «N secondi dopo che smetti di parlare»**, e quei 3 secondi che si sentono sono di Chrome, non nostri | la ② è il **passo B**, deciso: nuovo timer su `speechend`, valore di partenza **1200 ms** in `config` (`afterSpeechTimeoutMs`). ⚠️ **Deve essere più corto di quello di Chrome o è inerte** |
