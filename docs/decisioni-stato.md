...
| | Passo | Perché adesso |
|---|---|---|
| **1.3** | **Passo 18 — le stringhe del markup** | ⚠️ **RICONTATE E FATTE IN PARTE il 2026-09-20 (1.3a): 95 occorrenze in `index.html`, 47 distinte; 54 spostate.** Restano 41, e **nessuna per dimenticanza**: 9 di onboarding/home (devono funzionare quando niente funziona), 7 del Pannello Admin (strumento, non studente), 15 **sovrascritte a runtime** (segnaposto), 6 della **mappa** e 4 varie. ⚠️ **1.3b — LA MAPPA.** `openEpisodeMap` è **sincrona** e non aspetta i testi: spostare le sue 6 vuol dire farla aspettare, cioè darle **una schermata d'errore in più**. È una decisione sul comportamento, non uno spostamento, e va chiesta. ✅ **1.3c FATTO il 2026-09-20:** censimento vero su `app/*.js`, guardando **dove la stringa arriva allo schermo** e non il letterale in sé — **28 punti**, di cui **nove spostate** in cinque file. `Pausa` stava in due posti (file dei testi + codice del toggle) e le due potevano divergere; `Risposta corretta: ` era duplicata in `match.js` e `speedmatch.js`. Il resto resta con motivo: Pannello Admin, `console.warn`, le due frasi di ultima istanza, e la home. ⚠️ **E una cosa trovata e non corretta:** `app/personalizza.js:253` tiene una **copia di ripiego** del testo (`entry.title || 'Attenzione'`) — la stessa famiglia già corretta in quel file con la frase «Niente copia del testo come fallback». Toglierla cambia cosa si vede quando la chiave manca, quindi è una decisione. **Condizione: 1.3c.** ⚠️ **E un'altra, che non è una stringa:** il badge di **Repeat Aloud** è l'unico degli otto moduli **senza id** (`<span class="badge">Repeat Aloud</span>`), quindi l'unico che nessuno sovrascrive col nome vero da `CONFIG.moduleLabels`. Gli altri sette ce l'hanno. *È il caso «chi manca di qualcosa che hanno tutti gli altri» della regola 42, trovato contando.* **Condizione: 1.3c.** |
