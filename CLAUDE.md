...
| `tests/tools/attendi-ci.sh` | L'attesa **sulla CI**: `tests/tools/attendi-ci.sh regressione.yml <commit>`. È il **parente** di `attendi.sh`, non il suo gemello — stessa idea, meccanismo diverso, e i due si citano a vicenda in testa (regola 30). Identifica la corsa per **file del workflow + commit**, non per nome: lo stesso workflow ha **due nomi diversi** nella stessa API. ⚠️ **Il commit va INTERO, e lo script lo estende da solo con `git rev-parse`**: `head_sha` confronta la stringa per intero, e un prefisso dà `total_count: 0` — cioè la stessa risposta di una corsa che non esiste. Un commit che resta corto esce **64 col motivo**, mai 3. Uscite: 0 verde · 1 finita male, e dice se è **rossa** o **annullata** · 2 viva e non finita · 3 **la corsa non esiste** · 4 **non riesco a vedere l'API**. *Il 3 e il 4 sono separati perché «il push non ha fatto partire la CI» e «io non vedo GitHub» sono due ricerche diverse.* |
...
3. **Nessun valore modificabile scritto fisso nel codice** — soglie numeriche, tempi, liste, percentuali. Tutto questo vive in `window.APP_CONFIG`, che sta in **`app/config.js`** — un tag `<script>` bloccante, il primo dei ventitré, quindi già in memoria quando parte tutto il resto. *Qui c'era scritto «in cima a `index.html`», ed è stato vero fino al 2026-09-19.* Aggiungere un nuovo parametro tunabile significa aggiungere una chiave lì, non un numero sparso nel codice. `APP_CONFIG` è la base dati del **Pannello Admin (config)**, che esiste già: si apre digitando `config` fuori da un campo di testo (o con `?config` nell'indirizzo), mostra un gruppo per ogni chiave di primo livello e scrive le modifiche negli override in `localStorage`. Aggiungere un parametro lì significa vederlo comparire nel pannello senza scrivere altro.
...
4. **Il contenuto didattico non va scritto dentro il codice dei componenti** — parole, frasi, traduzioni, spiegazioni. Va letto da file di dati esterni.
   ...
   - **Il grado che un modulo legge è deciso dalla sequenza, non dal modulo.** Una **sequenza** è una lista di coppie `{ module, grade }`: la coppia dice quale modulo e su quale grado lavora. Le sequenze vivono in **`data/{lingua}/{studente}/struttura-corso.json`**, una per nome (oggi solo `narrativo-standard`, i ventidue passi), e sono **dell'edizione**: un corso di francese ne ha di sue, quante ne servono.

     *Dal 2026-09-20 (passo 1.11b). Prima stavano in `CONFIG.sequences`, cioè in un valore globale unico — il che voleva dire che una decisione presa per l'inglese avrebbe governato il francese in silenzio, l'opposto di quello che dice questa stessa regola sulle edizioni indipendenti. Il file arriva prima di qualunque schermata e i suoi valori tornano su `CONFIG`, quindi **chi legge `CONFIG.sequences` nel codice continua a scrivere la stessa riga**: è cambiato da dove vengono, non dove si trovano a runtime.* Nel descrittore di `EPISODES.<episodio>.modulesById` il grado NON c'è — lì sta solo ciò che non dipende da dove il modulo è messo (`kind`, `dataFile`, direzione, profilo dialogo, categoria). Cambiare grado a un passo è cambiare una lettera nella coppia, niente altro.

     **Ogni episodio dichiara la propria sequenza, sempre, anche il primo**: `episodes.<id>.sequence` del file di struttura porta il nome. ⚠️ **E la direzione è questa, non l'inversa: è l'EPISODIO che dichiara la sequenza, non la sequenza che elenca i suoi episodi.** Una sequenza non sa niente di chi la usa, e due episodi possono chiedere la stessa. Non esiste una sequenza di default che qualcuno eredita in silenzio — `moduleOrderDefault` è stato tolto apposta il 2026-09-08, perché il primo episodio corto avrebbe preso i ventidue passi narrativi senza che nessuno l'avesse deciso.
...
30. **Lo "Sblocco Sequenziale" è un meccanismo con un nome, e ha TRE varianti che restano separate.** L'idea è una: più avanti non si va finché non si è fatto qui. Le tre varianti sono **per ascolto** (Ripeti a Tempo — `dgApplySequenceLock`), **per dichiarazione** (Why We Say It — `storyCardsRefreshExplanationStates`) e, dal 2026-09-22 (passo 1.13-bis), **per completamento** (la lista degli episodi — `episodeStatus`).

    ⚠️ **La terza lavora un livello sopra le altre due: non fra i passi di un modulo, ma fra gli EPISODI di un corso.** E decide guardando **l'ULTIMA POSIZIONE** della sequenza, non un modulo chiamato per nome e non un conteggio. *Gli episodi di `grammatica` avranno meno moduli e nessun grado D: legare lo sblocco a un modulo romperebbe ogni episodio che quel modulo non ce l'ha, e un conteggio ri-bloccherebbe di colpo tutti gli episodi finiti il giorno in cui si aggiunge un passo in mezzo a una sequenza. **L'ultima posizione esiste in ogni sequenza, qualunque cosa contenga, e un passo aggiunto prima di lei non tocca niente.***

    Condividono l'idea e il nome, non il markup né il CSS: nella prima un passo avanti è una bolla sbiadita e inerte, nella seconda una card che resta leggibile, mostra il titolo della regola che aspetta e dice col lucchetto perché non si tocca, nella terza una riga di lista spenta e non cliccabile — **la stessa `.module-row` della mappa**, perché due liste che si somigliano devono somigliarsi anche nel codice o divergono al primo ritocco (regola 11). Unirle in un componente solo produrrebbe due varianti senza niente in comune se non il nome di una funzione. **Chi ne tocca una guardi l'altra**: i due punti del codice si citano a vicenda apposta.
