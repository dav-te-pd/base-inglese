...
A('|---|---|---|---|')
A('| `speedRoundMessages` | **15** | ⚠️ **NON HA MAI AVUTO UN LETTORE, verificato in tre modi il 2026-09-26.** ① `app/speedmatch.js:328` pesca da **`moduleCompleteMessages`**, e lo fa **dal commit in cui e\' nato quel sottotitolo** (`d4dd507`). ② `git log -S\'speedRoundMessages[\'` e `-S\'.speedRoundMessages\'` su tutta la storia trovano **zero letture**. ③ Le due sole apparizioni nel codice erano la **copia inline di sicurezza** in `index.html` — aggiunta con `f7298f9`, sparita col fallback (`97b7d02`) — cioe\' un doppione dei dati, non un lettore. *Il nome e\' quello di prima della rinomina `speedRound` -> `speedMatch`: porta il nome di un modulo che non si chiama piu\' cosi\'.* | **da decidere:** cancellarla, **oppure** collegarla se Speed Match deve avere messaggi SUOI invece di quelli generici |')
A('| ⚠️ *e questa famiglia ERA GIA\' STATA TROVATA* | — | **Il censimento di `docs/validazione.md` l\'aveva scritta: «Mai letta. Unica occorrenza in `index.html`: la copia di sicurezza».** Quel documento il 2026-09-24 e\' finito in **`docs/archivio/`** perche\' descriveva un `index.html` da 11.000 righe. *L\'archivio ha fatto il suo mestiere — una fotografia non si aggiorna — ma nessuno ha estratto le cose ANCORA VERE prima di archiviarla, e la scoperta e\' andata via col documento.* | **la lezione e\' di processo:** prima di archiviare un documento se ne tirano fuori i fatti vivi |')
A('| `episodeFinalMessages` | **25** | **Aspetta un modulo che non esiste**: il **Test di verifica finale**, fra i «previsti ma non ancora costruiti» di `CLAUDE.md`. ⚠️ **E la differenza con quella sopra e\' precisa: qui una funzione compagna ESISTEVA** — `episodeFinalOutcomeCase`, che decideva QUALE dei tre gruppi usare — **ma non l\'ha mai chiamata nessuno**, e il passo F.5 l\'ha cancellata il 2026-09-21. *Nessun codice ha mai pescato una stringa da qui: c\'era chi sapeva scegliere il gruppo, non chi mostrava il testo.* Il blocco del test si chiama apposta **«Modulo Finale prep»** | **si tiene** (deciso il 2026-09-26): e\' contenuto scritto in anticipo di proposito. *Accorciarlo adesso e\' lavoro che nessuno puo\' collaudare — si rilegge quando il modulo si costruisce, con la schermata davanti* |')
A('')
A('*La differenza fra le due sta in una domanda sola: **qualcuno ha mai provato a')
A('mostrarle?** Nessuna delle due ha avuto un lettore vero, ma la prima non lo ha')
A('mai cercato — il suo modulo pesca altrove da sempre — e la seconda aspetta un')
A('modulo che non c\'e\'. **La prima e\' un residuo, la seconda un anticipo, e si')
A('somigliano solo guardando il conteggio.***')
A('')
open(OUT, 'w').write('\n'.join(O))
