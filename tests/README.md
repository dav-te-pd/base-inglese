...
`test_conta_attese.js`, `test_modulo_pronto.js`, `test_comportamento_audio.js`). Insieme costituiscono la
...
| `tests/test_ui_condivisa_estratta.js` | Che **l'interfaccia che un modulo indossa** — i due overlay, i testi letti dal file delle istruzioni, il sottotitolo d'esito, le stelle — viva fuori da `index.html` **e non si porti dietro lo stato di sessione**. ⚠️ **La riga che vale più delle altre è `[B]`, il confine:** la prova che tiene non è che le funzioni ci siano, è che il file **non nomini `currentEpisode`, `currentValues`, `pendingMastery`**. `itemText` è rimasta fuori per questo — una riga sola, che però lega i due globali di sessione — mentre `fillTemplate` è entrata perché li prende **come parametri**. *La differenza fra le due è tutta lì: una li riceve, l'altra li va a prendere.* ⚠️ **E il guasto MUTO, che è quello che serve davvero:** i quattro `addEventListener` di chiusura sono venuti col file. Senza di loro **nessun errore JS** — gli overlay semplicemente non si chiudono più. Nessuna asserzione sulla navigazione se ne accorgerebbe. Visto fallire su **tre guasti**: tag spostato in `<head>`, cioè prima di `dati.js` (1 rossa in `test_dipendenze_dichiarate`, e **nomina le due dipendenze rotte**); `itemText` portata dentro (2 rosse); i quattro listener tolti (1 rossa, e dice **0**). ⚠️ **La terza falsificazione ha dovuto essere rifatta:** la prima volta non aveva tolto niente — il `sed` cercava due spazi di rientro dove ce n'erano quattro — e il test restava verde per l'assenza del guasto, non per debolezza. *Presa perché il rosso era stato previsto* (⓪-nonies). **Limite dichiarato:** non verifica che gli overlay si chiudano con **Escape**. Quel listener chiude *tre* overlay e il terzo è del Pannello Admin, quindi è rimasto in `index.html`: la sua riga la scrive chi estrarrà l'Admin. |
| `tests/test_orchestrazione_estratta.js` | Che il meccanismo delle viste viva fuori da `index.html` **e che il suo tag stia dove deve**. ⚠️ **`app/orchestrazione.js` è il primo strato della SECONDA FILA:** prende i tredici nodi delle viste con `getElementById` **a tempo di parsing**, quindi sta in fondo a `<body>` e non in `<head>` con gli altri nove. Spostato in `<head>` — dove uno andrebbe a metterlo per simmetria — i tredici diventano `null` e l'app non arriva alla schermata del nome. *Un guasto grosso che nasce da una modifica che sembra un riordino innocuo.* Le tre righe `[A]` fissano la posizione in tre modi che **non si coprono a vicenda**: dopo il markup, prima dello script principale (senza questa, il tag in fondo alla pagina passerebbe), e non in `<head>`. **Il caso più diverso (regola 42):** `pronunciation`, la vista **morta** — nessun modulo la apre, quindi è l'unica delle tredici che nessun cammino dell'app tocca; un elenco che la perdesse resterebbe verde su ogni test di navigazione. Per questo `[C]` conta i nodi invece di navigare. Visto fallire su due guasti: tag in `<head>` (**6 rosse**, e il `pageerror` dice `Cannot read properties of null`), una vista tolta dall'elenco (2 rosse, e dicono 12). **Limite dichiarato:** fissa la posizione di *questo* tag, non che ogni futuro strato della seconda fila ci stia — quando uscirà `ui-condivisa`, la sua riga la scrive il suo test. |
...
| `test_comportamento_audio.js` | Che il Blocco Ascolto **parli al tocco** e che il microfono **parta, si fermi al click e si spenga da solo** — cioè il *comportamento*, non la presenza. **Cosa si perde senza, ed è un caso vero del 2026-09-19:** uno spostamento di sette pezzi condivisi in `app/ui-condivisa.js`, con funzioni identiche carattere per carattere e diff pulito, ha fatto rispondere il Blocco Ascolto con ~3 secondi di ritardo e il microfono a non fermarsi al click — **e nessuna delle 1445 asserzioni se n'è accorta**, perché tutte quante quei pezzi li *cercavano* (la funzione esiste, il markup è in un punto solo, il modulo si apre) e nessuna li *guidava*. Un'estrazione può lasciare tutto raggiungibile e cambiare l'ordine in cui le cose girano — e l'ordine è esattamente ciò che uno studente sente. **Come, e sono due scelte:** ① il «subito» non si cronometra — click e lettura stanno **dentro la stessa chiamata sincrona**, quindi se `speak` parte nel gestore è già registrato quando la riga dopo lo legge, e la misura non dipende da quanto è veloce la macchina (regola 19); ② il finto sintetizzatore dichiara `speaking` e il finto microfono registra `start`/`stop`/`abort` invece di ingoiarli. ⚠️ **Quei due finti sono la scoperta del file**: `test_voicecoach.js` — il test del modulo che il microfono ce l'ha — ha `stop() {}` **vuoto** e un sintetizzatore **senza `speaking`** (misurati: 17 finti su 41 non dichiarano `speaking`, 4 file hanno lo `stop()` vuoto; l'elenco e i comandi stanno in `docs/decisioni.md`) — su di loro `fermaLaVoce()` non chiama `cancel()` mai, quindi «il secondo tocco ferma la voce» e «il microfono si ferma al click» erano guasti che quei finti **non potevano mostrare** (regola 19: un mock che semplifica troppo dà una sicurezza falsa). **Visto fallire su tre guasti realistici**, ognuno con la sua previsione scritta prima: il Blocco Ascolto rinviato di un tick (4 righe rosse), il `recognition.stop()` tolto dal secondo click (1 riga), il taglio per silenzio disattivato (2 righe). **E visto NON fallire dove doveva:** con gli stessi guasti `test_blocco_ascolto.js` resta 19/19, `test_voicecoach.js` 26/26 e `test_avviso_microfono.js` 31/31 — è questa la prova che il file aggiunge qualcosa invece di ripetere. **Limite dichiarato:** guida Meet the Story e Voice Practice; gli altri quattro moduli passano dalla stessa `speakListenBlock` ma qui non sono guidati. E dei tre sintomi visti su Pages ne copre **due** — «spesso il microfono non sente» non è riproducibile con un riconoscimento finto, che sente sempre. |
