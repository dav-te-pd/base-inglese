...
| `tests/attese.js` *(non è un test: è il magazzino delle attese)* | Che un'attesa condivisa esista in **un posto solo**. Oggi: `attendiSottotitoloEsito` (un testo che arriva da un fetch), `attendiVisibile` e `attendiNascosto` (una schermata che compare o sparisce). Tutte e tre **ritornano `true`/`false` invece di sollevare**, così lo scadere del tempo diventa l'asserzione che fallisce, con il suo testo, invece di un `TimeoutError` muto. **Limite dichiarato** su `attendiNascosto`: per Playwright «hidden» è vero anche per un elemento che **non esiste più**, quindi dove serve dire «c'è ma è nascosto» il controllo sull'esistenza resta al chiamante. |
