...
`test_report_mastery.js`, `test_episodi_corti.js`, `test_sequenze.js`, `test_episodio2.js`). Insieme costituiscono la
...
| `test_episodio2.js` | Che `data/it/a1-episodio2-inglese.json` continui a dire quello che dichiara `docs/it/episodio-2.md`. Oggi quel file **non lo legge nessuno** — l'episodio 2 non è in `EPISODES` — ed è proprio per questo che può divergere senza che niente lo mostri: nessuna schermata sbaglia, nessun modulo si apre vuoto, c'è solo un file che invecchia in silenzio finché qualcuno lo collega. Confronta i numeri dichiarati e le due cose che un conteggio non vede: che ogni skill stia su una battuta esistente e che ogni `fromLine` punti a una battuta vera. **Limite dichiarato:** il testo NON è confrontato — i due markdown hanno forme diverse e un secondo parser sarebbe più fragile di quanto protegga. Quando l'episodio 2 entrerà in `EPISODES`, il confronto testuale va esteso a lui. |
