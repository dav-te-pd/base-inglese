...
const bootAsUser = (page, userName, completedModules) =>
  bootUtente(page, { utente: userName, completati: completedModules, introChiuse: ['mappaEpisodio'], personalizzaVista: true });
...
async function run() {
  ...
  // ============ Regression: mod1 (Ascolta e Ripeti) still works after dgLockAll/dgPlayLine unification ============
  {
    ...
    const alzata = await page.evaluate((id) => {
      ...
      b.click();
      return b.classList.contains('is-active') && window.speechSynthesis.speaking === true;
    }, D1);
    log('[Regression] Mod1 alza la bolla che sta parlando', alzata === true);
