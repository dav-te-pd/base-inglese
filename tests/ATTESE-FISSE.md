...
| `test_dialogo_extra.js` | 10 | [Regression] Mod1 lifts the playing bubble but no longer locks others (free-tap profile) | verifica che una cosa NON accada: un non-evento non si aspetta, il tempo E' la misura — nel profilo a tocco libero le ALTRE bolle NON devono bloccarsi. ⚠️ **Il 2026-09-24 l'asserzione e' stata SPEZZATA:** leggeva anche `is-active`, che il codice mette quando l'audio PARTE — dieci millisecondi dopo il click erano una corsa, e l'ha persa. Adesso la prima meta' ASPETTA quella classe ed e' un'asserzione sua; il tempo resta qui, dove misura un non-evento, **e parte da un istante noto invece che dal click** |
